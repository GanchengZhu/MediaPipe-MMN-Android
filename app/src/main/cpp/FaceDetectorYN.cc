...
namespace mirror {
    ...
    bool FaceDetectorYN::LoadModel(AAssetManager *mgr, const char *model_file) {
        std::cout << "Start load model." << std::endl;
        // 1.load model
//  net_ = std::unique_ptr<Interpreter>(Interpreter::createFromFile(model_file));
        // 打开模型文件
        AAsset *modelAsset = AAssetManager_open(mgr, model_file, AASSET_MODE_BUFFER);
        if (modelAsset) {
            // 读取模型文件数据
            const void *modelData = AAsset_getBuffer(modelAsset);
            off_t modelSize = AAsset_getLength(modelAsset);
            net_ = std::unique_ptr<Interpreter>(
                    Interpreter::createFromBuffer(modelData, modelSize));
            // 现在你可以使用 modelData 和 modelSize 来处理模型文件的数据

            AAsset_close(modelAsset); // 关闭文件
        }
    ...
    bool FaceDetectorYN::Detect(const ImageHead &in, RotateType type,
        ...
        for (size_t i = 0; i < stride_num; ++i) {
            ...
            MNN::Tensor *bbox = net_->getSessionOutput(sess_,
                                                       output_names_[i + stride_num * 2].c_str());
            MNN::Tensor *kps = net_->getSessionOutput(sess_,
                                                      output_names_[i + stride_num * 3].c_str());
            ...
            std::shared_ptr<MNN::Tensor> bbox_result(
                    new MNN::Tensor(bbox, bbox->getDimensionType()));
